...
class XmlUpdater:
    ...
    def get_updated_xml(self, changes: dict, source_xml_path: str, create_elements_whitelist: list, raise_on_missing_game: bool = True):
        ...
        if raise_on_missing_game:
            for game_id in changes:
                if game_id not in results:
                    raise self.GameNotFound(f"Unable to find game: \'{game_id}\'", game_id)
...
if __name__ == "__main__":
    ...
    updater = XmlUpdater()
    try:
        updated_xml, changed_games = updater.get_updated_xml(changes, f"{home}/Desktop/Unity.xml", ["Hide"])
    except Exception as e:
        print(str(e), e.game_id)
